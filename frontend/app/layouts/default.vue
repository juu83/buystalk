<script setup>
import { ref } from 'vue'
const { user, logout } = useAuth()
const isOpen = ref(false)
const closeAndLogout = () => {
  isOpen.value = false
  logout()
}
</script>

<template>
  <div class="min-h-screen bg-gray-50 font-sans text-gray-900">
    <nav class="bg-white/90 backdrop-blur-sm border-b sticky top-0 z-50">
      <div class="max-w-3xl mx-auto px-4 py-2 flex items-center justify-between gap-4">
        <NuxtLink to="/posts" class="flex items-center gap-3">
          <div class="bg-gradient-to-tr from-blue-500 to-indigo-600 text-white rounded-full w-9 h-9 flex items-center justify-center font-bold">S</div>
          <span class="font-extrabold text-base md:text-xl text-blue-600 tracking-tight">BuyStalk</span>
        </NuxtLink>

        <ul class="hidden md:flex items-center gap-4 font-medium text-gray-600 text-sm md:text-base">
          <li><NuxtLink to="/posts" class="hover:text-blue-600 transition px-2 py-1">Fil</NuxtLink></li>
          <li><NuxtLink to="/ads" class="hover:text-blue-600 transition">Annonces</NuxtLink></li>
          <li><NuxtLink to="/map" class="hover:text-blue-600 transition px-2 py-1">Carte</NuxtLink></li>
          <li><NuxtLink to="/account" class="hover:text-blue-600 transition px-2 py-1">Compte</NuxtLink></li>
          <li v-if="!user"><NuxtLink to="/login" class="text-blue-600 hover:text-blue-800 px-2 py-1">Se connecter</NuxtLink></li>
          <li v-else><button @click="logout()" class="text-gray-600 hover:text-red-600 px-2 py-1">Se déconnecter</button></li>
        </ul>

        <div class="flex items-center gap-1 md:hidden">
          <NuxtLink to="/posts" aria-label="Fil" class="p-2 rounded-md hover:bg-slate-800 text-white">🌍</NuxtLink>
          <NuxtLink to="/ads" aria-label="Annonces" class="p-2 rounded-md hover:bg-slate-800 text-white">📢</NuxtLink>
          <NuxtLink to="/map" aria-label="Carte" class="p-2 rounded-md hover:bg-slate-800 text-white">🗺️</NuxtLink>
          <NuxtLink to="/account" aria-label="Compte" class="p-2 rounded-md hover:bg-slate-800 text-white">👤</NuxtLink>
          <button @click="isOpen = !isOpen" aria-label="Menu" class="ml-2 p-2 rounded-md hover:bg-slate-800 text-white">
            <svg v-if="!isOpen" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
            <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
          </button>
        </div>
      </div>

      <div v-if="isOpen" class="md:hidden">
        <div class="max-w-3xl mx-auto px-4 py-2">
          <ul class="bg-white rounded-md shadow-sm divide-y">
            <li><NuxtLink @click="isOpen = false" to="/posts" class="block px-4 py-3 hover:bg-gray-50">Fil d'actualité</NuxtLink></li>
            <li><NuxtLink @click="isOpen = false" to="/ads" class="block px-4 py-3 hover:bg-gray-50">Petites Annonces</NuxtLink></li>
            <li><NuxtLink @click="isOpen = false" to="/map" class="block px-4 py-3 hover:bg-gray-50">Carte interactive</NuxtLink></li>
            <li><NuxtLink @click="isOpen = false" to="/account" class="block px-4 py-3 hover:bg-gray-50">Mon compte</NuxtLink></li>
            <li v-if="!user"><NuxtLink @click="isOpen = false" to="/login" class="block px-4 py-3 hover:bg-gray-50">Se connecter</NuxtLink></li>
            <li v-else><button @click="closeAndLogout" class="w-full text-left px-4 py-3 hover:bg-gray-50">Se déconnecter</button></li>
          </ul>
        </div>
      </div>
    </nav>

    <main class="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8">
      <slot />
    </main>
  </div>
</template>
